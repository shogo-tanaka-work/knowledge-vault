...
- 対象サイト1: [https://www.anthropic.com/news](https://www.anthropic.com/news)
- 対象サイト2: [https://claude.com/ja-jp/blog](https://claude.com/ja-jp/blog)
- 対象サイト3: [https://www.canva.com/newsroom/（Cloudflare保護）](https://www.canva.com/newsroom/%EF%BC%88Cloudflare%E4%BF%9D%E8%AD%B7%EF%BC%89)
...
- ⚠️ 長時間タスクは30-120秒かかる（リアルタイム処理には不向き）

### 操作性・UI/UX

- ✅ CLIコマンドがシンプルで直感的
- ✅ エラーメッセージがわかりやすい
- ⚠️ セッションのクリーンアップを手動で行う必要がある
- ⚠️ ブラウザプロセスがDockに残る問題（try-finallyで対処可能）
...
- ✅ 日付フィルタリングも正確に動作
- ✅ 重複除去により精度が向上
- ⚠️ Cloudflare対策として30秒待機が必要（自動化に影響）
...
- ⚠️ スクレイピング対象サイトのrobots.txt確認が必要
- ⚠️ 利用規約違反にならないよう注意
- ⚠️ 個人情報を含むページの自動取得は慎重に
...
- ⚠️ Cloudflare等のボット対策が強化されると突破困難に
- ⚠️ サイト構造変更で抽出ロジックが壊れる可能性
- ⚠️ Python 3.11依存により、古い環境では使用不可
...
extractor = RecentNewsExtractor(days=7)

articles = extractor.extract_news_from_anthropic("https://www.anthropic.com/news")
```
...
"title": "Claude 4.5 Released with Enhanced Capabilities",

"href": "https://www.anthropic.com/news/claude-4-5-release",
...
extractor = CanvaNewsExtractor()

articles = extractor.extract_canva_news()
```

**処理フロー**:

1. `-headed`モードでブラウザ表示
2. 30秒待機（Cloudflareチャレンジ自動解決）
3. ページタイトル確認（"Newsroom"が含まれるか）
4. HTML取得・パース
...
**シナリオ**: [https://claude.com/ja-jp/blog](https://claude.com/ja-jp/blog) から最新記事を取得

**特殊対応**:

- h2要素（`u-text-style-h6`クラス）からタイトル取得
- 次の兄弟要素（`caption`クラス）から日付取得
- grandparent要素内のリンクを検索
...
- 最初Python 3.10環境で実行したところ、依存関係エラーが発生
- requirements.txtで`browser-use>=0.12.0`と指定することで明示化
- `/usr/local/bin/python3.11`を直接使用することで解決

### セッション管理の重要性

- `-session`オプションでセッション名を指定できる
- 複数の並行実行が可能
- ただし、`close`コマンドを忘れるとブラウザプロセスがDockに残り続ける
...
# browser-use eval でJavaScript実行

result = self.run_command(["eval", "document.querySelectorAll('a')"])
```

**ユーザーフィードバック**:

> "eval実行はXSSなどのリスクが高いので使わないほうがいいかと"
> 
...
# browser-use get html + BeautifulSoup で安全にパース

result = self.run_command(["get", "html"])
...
**学び**: JavaScriptの動的実行は避け、HTML取得→静的パースの方が安全

---

### ✅ 日付抽出の精度向上

**問題**: April 2の記事が取得できない

**ユーザーフィードバック**:

> "[https://claude.com/ja-jp/blog](https://claude.com/ja-jp/blog)　のサイトはApr 2に1件あります。ハーネスに関する内容です"
> 

**原因調査**:

1. 日付パターンが省略形（Jan, Feb）のみで完全形（January, February）に対応していなかった
2. HTML構造が想定と異なり、grandparent要素内のリンクを見ていなかった
...
# grandparent要素のリンクも検索

grandparent = parent.find_parent()

if grandparent:

link = grandparent.find('a', href=re.compile(r'/(blog|research)/'))
```

**学び**: 日付フォーマットは複数パターン対応が必須、HTML構造は階層を上に辿る必要がある場合も
...
> "同じニュースを２個取得しているようです。これは実行過程での一時的なものでしょうか？"
> 

**原因**: カルーセル等で同じ記事が複数箇所に表示されている
...
print(f"【重複除去】{len(articles) - len(unique_articles)}件の重複を削除")
```

**学び**: スクレイピング結果は必ず重複チェックが必要

---

### ✅ Cloudflare保護の突破

**問題**: headlessモードでは"Just a moment"ページで止まる

**検証プロセス**:

1. **headlessモードで実行** → タイトルが"Just a moment"
2. **スクリーンショット確認** → Cloudflareチャレンジページ
3. **`-headed`モード + 30秒待機で再試行** → 成功
...
print("2. Cloudflareチャレンジの解決を待機（30秒）...")
...
if "Newsroom" in title:

print("✅ ページ読み込み成功")
```

**学び**: Cloudflare保護サイトはheadedモード + 待機時間で対応可能、ただし自動化には制約

---

### ✅ セッションクリーンアップの徹底

**問題**: ブラウザプロセスがDockに残り続ける

**ユーザーフィードバック**:

> "パソコンから開いてるのでPythonが画面下のメニューにずっと残り続けるっていう問題が起きてるんですけどこれはもうOSそのものの問題なんでしょうか?"
> 
...
["browser-use", "--session", cli.session_name, "close"],
...
**学び**: try-finallyパターンでリソースの確実な解放を保証

---

### うまくいかなかったこと

### ❌ Cloudflare自動突破の完全自動化

- headlessモードでは100%ブロックされる
- headedモード + 30秒待機でも、時々失敗することがある
- 完全自動化には限界があり、人間の介入前提の設計が必要
...
session_name = f"{purpose}_{timestamp}" # 例: "news_extraction_20260405"
```
...
result = self.run_command(["get", "html"], timeout=180)
```
...
### エラー1: Python version not supported

**原因**: Python 3.10以下を使用している

**対処**: Python 3.11以上にアップグレード

### エラー2: Session already exists

**原因**: 前回のセッションが残っている

**対処**: `browser-use --session XXX close`を実行

### エラー3: Timeout expired

**原因**: タイムアウト設定が短すぎる

**対処**: `timeout`パラメータを120-180秒に増やす

### エラー4: Command not found: browser-use

**原因**: PATHが通っていない

**対処**: `~/.zshrc`に`export PATH="$HOME/.local/bin:$PATH"`を追加
...
- GitHub: [https://github.com/browser-use/browser-use](https://github.com/browser-use/browser-use)
- CLI Documentation: [https://browser-use.com/docs/cli](https://browser-use.com/docs/cli)
- Python API: [https://browser-use.com/docs/api](https://browser-use.com/docs/api)

### 参考記事・事例

- mcp-browser-use: [https://github.com/Saik0s/mcp-browser-use](https://github.com/Saik0s/mcp-browser-use)
