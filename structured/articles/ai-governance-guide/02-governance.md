...
created: 2026-05-08
type: study-notes
source: 野口侑渡『生成AI完全ガイド』第2回オフラインセッション
tags: [ai-governance, generative-ai, legal, compliance]
---

# 第2回 生成AIの統制（ガバナンス）— 条件付きで「使える」状態を設計する

> 出典: 野口侑渡氏（大手IT企業 生成AI推進担当 / Fluxia代表）「生成AI完全ガイド 第2回: 生成AIの統制」オフラインセッション資料より要点整理。
...
第2回は **3パートの入れ子構造** で展開される。これがこの講座の最も独創的なロジック設計。

```mermaid
flowchart LR
  R[Part 1 リスク<br/>何が起き得るか<br/>事例ベース]
  L[Part 2 法律<br/>適法/違法の境界線<br/>判断基準]
  G[Part 3 ガバナンス<br/>誰が・何を・どこまで<br/>組織設計]
  R --> L --> G
```
...
講義は架空の中堅M&A仲介会社「クロスキャピタル」（600名、年間120件、扱う案件30〜100億円）を題材に進む。情報システム部長 山田氏が直面しているのは：
...
![図: AI利用5リスク領域（第2回 p.7 より）](./images/02-risk-map.png)
...
mindmap
  root((AI利用<br/>5リスク領域))
    ★情報漏洩<br/>LEAKAGE
      ...
      機密データ入力
      最も発生頻度高
    ★ハルシネーション<br/>HALLUCINATION
      事実無根の生成
      誤判断・責任問題
    ★知財・著作権侵害<br/>IP INFRINGEMENT
      他者著作物の意図しない生成
      自社生成物の権利保護
    セキュリティ
      プロンプトインジェクション
...
```mermaid
timeline
  title Samsung社内ChatGPT事件のタイムライン
  2023.03 : 社内利用を許可
  2023.04 : わずか数週間で3件のインシデント発生<br/>(ソースコード×2、会議録音内容)
  2023.05 : 全面禁止 → 社内AI開発着手
  2023.11 : Samsung Gauss発表
```
...
![図: 著作権の境界線 — 学習段階(基本セーフ) vs 生成・利用段階(リスク)（第2回 p.16 より）](./images/02-copyright-boundary.png)

```mermaid
graph LR
  subgraph 学習段階[学習段階]
    L1[著作権法 30条の4<br/>非享受目的の情報解析<br/>原則適法]
  end
  subgraph 生成段階[生成・利用段階 ★リスクの本丸]
    G1[侵害要件:<br/>依拠性 + 類似性]
  end
  学習段階 --> 生成段階
```
...
```mermaid
graph TB
  L1[層1: 競争優位の喪失<br/>独自ノウハウ・技術的優位性の毀損]
  L2[層2: 契約違反<br/>NDA・秘密保持義務違反]
  L3[層3: 不正競争防止法違反<br/>営業秘密3要件を満たす情報の漏洩]
```
...
graph TB
  S{契約関係の有無}
  S -->|あり| C[債務不履行責任<br/>民法644条 善管注意義務違反<br/>+ 415条]
  S -->|なし| T[不法行為責任<br/>民法709条 過失]
  C --> Core[核心: 検証義務違反]
  T --> Core
```
...
flowchart LR
  subgraph INPUTS[INPUTS]
    R[AIリスク 5領域]
    Law[法規制・ガイドライン<br/>著作権法30条の4<br/>不正競争防止法<br/>個人情報保護法<br/>民法418/722条<br/>AI利活用促進法]
  end
  subgraph DESIGN[INTEGRATION 意思決定]
    P[ポリシー策定]
    SOP[標準手順 SOP]
    Tool[許可ツール選定]
    Log[監視・記録要件]
...
![図: Who/What/Where 3軸の設計フレーム（第2回 p.25 より）](./images/02-3axis.png)
...
```mermaid
graph LR
  Who[Who 誰が<br/>推進体制] --> Output[ガバナンス成果物]
  What[What 何を<br/>ルール・スコープ] --> Output
  Where[Where どこまで<br/>環境・監査] --> Output
```

### 3社のガバナンス型比較

![図: 3社のガバナンス型一覧（第2回 p.26 より）](./images/02-three-types.png)
...
→ 前: [第1回 生成AIの全体像](./01-overview.md) ｜ 次: [第3回 生成AIの導入戦略](./03-strategy.md)
