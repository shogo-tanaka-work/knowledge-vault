# Claude Code + GitHub + Vercel でつくるAI駆動開発の環境（Windows版）

> ゼロから本番公開まで解説

> **macOS版との違い：** STEP 1（開発環境のセットアップ）のみ異なります。STEP 2 以降は共通です。
...
## STEP 1: 開発環境のセットアップ（Windows）

必要なツールを4つインストールします。Windows では **winget**（Windows標準のパッケージ管理ツール）を使うのが最も手軽です。

> **winget とは？** macOS の Homebrew に相当する、Windows 標準のコマンドラインインストーラーです。Windows 10 21H1 以降・Windows 11 に標準搭載されています。

### 1-0. 使用するターミナル

**PowerShell**（Windows に標準搭載）を使います。スタートメニューで「PowerShell」と検索して起動してください。

より快適に使いたい場合は **Windows Terminal**（Microsoft Store から無料でインストール可能）もおすすめです。

### 1-1. Git for Windows

バージョン管理ツール Git を Windows にインストールします。

```powershell
winget install Git.Git
```

インストール後、PowerShell を再起動してからバージョン確認してください。
...
インストール後、PowerShell を再起動してから認証します。
...
> **ポイント：** winget でインストールしたツールはすべて、PowerShell を **再起動してから** バージョン確認するようにしてください。再起動しないと「コマンドが見つからない」エラーが出ます。
...
`.github/workflows/ci.yml` を作成するだけです。CI はクラウド（GitHub のサーバー）で動くため、Windows・macOS 問わず同じ設定ファイルで動作します。
...
コードができたら Vercel で公開します。GitHub と連携するだけで、main へのマージが自動で本番反映になります。Vercel の操作はすべてブラウザ上で行うため、OS による違いはありません。
...
以下は変数の参考例です。PJに応じて調整してください。
APIキーやDBサーバの認証情報などが追加対象となります。
...
| GitHub CLI インストール | `brew install gh` | `winget install GitHub.cli` |
| Node.js インストール | 公式サイト or brew | `winget install OpenJS.NodeJS.LTS` |
| Git インストール | 標準搭載 | `winget install Git.Git` |
| ターミナル | Terminal.app / iTerm2 | PowerShell / Windows Terminal |
| sudo（権限昇格） | `sudo` | 「管理者として実行」 |
| STEP 2以降 | 共通（ブラウザ操作） | 共通（ブラウザ操作） |
