...
- 2026/05/28: 「CU のデスクトップ操作 = 実質 Linux デスクトップ」「Windows/Mac 業務アプリは別途 VM 必要」「業務別ユースケースカタログ」を整理。詳細は Anthropic ログ（`20260527_anthropic-computer-use.md`）の §5・§8・§9 に集約。OpenAI は環境設定で linux/mac/windows/ubuntu/browser を切り替えられるが、コンテナ環境は自前で用意する建付け。Windows 環境への実機検証は実施しない判断
