...
> ステータス: 完了 作成日: 2026/04/15 最終更新: 2026/04/15 ファイルパス: ~/Documents/verification-logs/tools/20260415_github-remote-mcp-verification.md
...
> このセクションを読めば、同じ環境を最初から再現できる。 所要時間の目安：PAT発行〜claude.ai接続完了まで **約3〜4時間**（初回）
...
- WorkerはGitHub APIをPATで呼び出す（PAT = GitHubへの書き込み権限を持つ鍵）
- **「OAuthで書き込めない」問題を、Worker側でPATに変換することで回避する構成**

---

### STEP 1：Fine-grained PATの発行（GitHub）

GitHub APIの書き込みに必要なPATを発行する。
...
|項目|設定値|
|---|---|
|Token name|任意（例: `github-mcp-worker-pat`）|
|Expiration|任意（90日推奨）|
|Repository access|対象リポジトリのみ（All repositories は避ける）|

4. **Permissions** で以下を `Read and write` に設定：
...
5. **Generate token** → 表示されたトークン文字列をコピーして安全な場所に保管

> ⚠️ `Read only` のままだと `create_branch` や `create_pull_request` が403になる

---

### STEP 2：Client ID / Client Secret の生成

claude.aiのOAuth接続に使うキーペアを **ローカルのJSスクリプト** で生成する。

```js
// generate-oauth-keys.js
// Node.js で実行: node generate-oauth-keys.js

const { webcrypto } = require('crypto');

async function generateKeys() {
  // Client ID: ランダムな文字列（32バイト）
  const clientIdBytes = webcrypto.getRandomValues(new Uint8Array(32));
  const clientId = Buffer.from(clientIdBytes).toString('hex');

  // Client Secret: ランダムな文字列（64バイト）
  const secretBytes = webcrypto.getRandomValues(new Uint8Array(64));
  const clientSecret = Buffer.from(secretBytes).toString('hex');
...
```bash
node generate-oauth-keys.js
# → CLIENT_ID=  xxxx...
# → CLIENT_SECRET= yyyy...
```

> この `CLIENT_ID` と `CLIENT_SECRET` を次のSTEPでWorkerのシークレットに登録し、 claude.aiのコネクタ設定でも同じ値を入力する。

---

### STEP 3：Cloudflare WorkerへのデプロイとPAT登録

#### 3-1. リポジトリのフォークとクローン

```bash
# GitHubで公式リポジトリをフォーク後
git clone https://github.com/[自分のアカウント]/github-remote-mcp-server.git
cd github-remote-mcp-server
```

#### 3-2. Wranglerでシークレットを登録

```bash
# GitHubのFine-grained PATを登録
echo "ghp_xxxxxx..." | wrangler secret put GITHUB_PAT

# STEP2で生成したClient IDとClient Secretを登録
echo "xxxx..." | wrangler secret put CLIENT_ID
echo "yyyy..." | wrangler secret put CLIENT_SECRET
```

> Cloudflare Workersのシークレットはダッシュボードから登録する方法もある： Workers & Pages → 対象Worker → **Settings** → **Variables and Secrets** → **Add**

#### 3-3. User-Agentヘッダーの追加（必須）

GitHub APIは `User-Agent` ヘッダーがないと403を返す。 Workerのコードで `fetch` を使っている箇所に以下を追加する：
...
```bash
wrangler deploy
# → https://github-mcp-server.[サブドメイン].workers.dev にデプロイされる
```

デプロイ後、エンドポイントURLをメモしておく（例: `https://github-mcp-server.s-tanaka-dcb.workers.dev/mcp`）

---

### STEP 4：claude.ai カスタムコネクタの設定

1. claude.ai → **Settings**（歯車アイコン）→ **Integrations** または **Connectors**
2. **Add custom connector** をクリック
3. 以下を入力：

|項目|入力値|
|---|---|
|MCP Server URL|`https://[Worker名].[サブドメイン].workers.dev/mcp`|
|Client ID|STEP2で生成した `CLIENT_ID` の値|
|Client Secret|STEP2で生成した `CLIENT_SECRET` の値|

4. **Connect** → OAuth認証フローが走り、接続が完了する
5. チャット画面でコネクタが有効になっていることを確認

> ⚠️ **JWTトークン認証はclaude.aiのカスタムコネクタUIと非互換**。 必ず「Client ID + Client Secret」形式のOAuth認証を使うこと。 `Authorization: Bearer [JWT]` を直接入力できるUIは現状存在しない。

---

### STEP 5：動作確認

新しいチャットを開いて、以下の順番で疎通を確認する。

```
① debug_echo で疎通確認
「debug_echoで message="テスト" を送って」
→ {"echo": "テスト", "timestamp": "..."} が返ればOK

② 読み取り確認
「shogo-tanaka-work/shogo-works のIssue一覧を取得して」

③ 書き込みフルサイクル確認
「Issue #[番号] に対応するブランチを作成して、
 docs/test.md にログを追記して、PRを作成して」
```
...
|`403`（GitHub API初回呼び出し時）|`User-Agent` ヘッダー未設定|WorkerのfetchにUser-Agentを追加してwrangler deploy|
|claude.aiでコネクタが認証エラー|Client ID/SecretがWorkerシークレットと不一致|`wrangler secret list` で登録済みキー名を確認し、claude.ai側と一致させる|
|新しいツールがclaude.aiで認識されない|MCPツール一覧のキャッシュ|MCP接続をオフ→オン、または新しいチャットを開く|
|`Session terminated (32600)`|tools/call時にWorkerがタイムアウト|Streamable HTTP statelessモードへの移行を検討|
...
| 2026/04/15 | update — セクション3-Bとして再現手順（セットアップガイド）を追加。PAT発行・Client ID/Secret生成・Workerデプロイ・claude.ai接続の全STEP記載 |